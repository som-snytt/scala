    ...
    protected def adapt(tree: Tree, mode: Mode, pt: Type, original: Tree = EmptyTree): Tree = {
      ...
      def adaptMethodTypeToExpr(mt: MethodType): Tree = {
        ...
        def checkCanAutoApply(): Boolean = {
          def isJavaOrUniversal(sym: Symbol) = sym.isJavaDefined || isUniversalMember(sym)
          if (sourceLevel2_14 && !isJavaOrUniversal(meth) && !meth.overrides.exists(isJavaOrUniversal))
