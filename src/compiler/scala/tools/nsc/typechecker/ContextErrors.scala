      ...
      def NotAMemberError(sel: Tree, qual: Tree, name: Name, cx: Context) = {
        ...
        def errMsg: String = {
          ...
          def addendum         = {
            ...
            val javaRules = orEmpty(owner.isJavaDefined) {
              def more = orEmpty(settings.explaintypes)(sm""" Static Java members belong to companion objects in Scala;
                |they are not inherited, even by subclasses defined in Java.""")
              val (jtype, jmember) = cx.javaFindMember(target, name, _ => true)
              if (jmember != NoSymbol) s"did you mean ${jtype.typeSymbol.fullName}.${jmember.name}?$more" else ""
            }
            List(companion, altStr, notAnyRef, semicolon, javaRules).filter("" != _).mkString("\n", "\n", "")
          }
